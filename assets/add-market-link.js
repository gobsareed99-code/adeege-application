(()=>{'use strict';const API='https://tjwoqvqfavxqykvbasaf.supabase.co',KEY='sb_publishable_3XeoEp9EqRqPLPIm5Cwthg_Pf1lEYUZ';let storeMap=null,decorating=false;
const loadScript=(id,src)=>{if(document.getElementById(id))return;const s=document.createElement('script');s.id=id;s.src=src;s.defer=true;document.body.appendChild(s)};
const addAdminLink=()=>{const nav=document.querySelector('.nav .container');if(nav&&!document.getElementById('addMarketNav')){const a=document.createElement('a');a.id='addMarketNav';a.href='shops-admin.html?v=6';a.textContent='🏪 Add Market';a.style.fontWeight='800';nav.appendChild(a)}};
...
function start(){
  ...
  addAdminLink();decorateStores();
  const grid=document.getElementById('storesGrid');if(grid){let timer;new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(decorateStores,80)}).observe(grid,{childList:true})}
}
